...
# by using the facet_wrap command. 
...
# recorded data in the COVID data set is on Sep 27th. Use the filter command to filter
# and geom_violin added to ggplot.

LastMonthInfections <- InfectionGrowth %>%
  filter(date >= 20200827)

CpmViolin <- ggplot(LastMonthInfections, aes(State, CPM, fill = State)) +
...
PAcases <- LastMonthInfections %>%
  filter(State =="PA")

t.test(OHcases$CPM, PAcases$CPM)
